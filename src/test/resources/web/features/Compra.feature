...
Feature: Compra completa
  ...
  @CompraCheckoutCompletarCamposFallido
  Scenario Outline: El usuario completa sus datos en la pagina del Checkout
    And el usuario agrega el producto Sauce Labs Backpack al carrito
    And el usuario agrega el producto Sauce Labs Bolt T-Shirt al carrito
    And el usuario hace click en el carrito
    And el usuario esta en la pagina del carrito
    And el usuario hace click en el boton de Checkout
    And el usuario se encuentra el la pagina de Chekout
    When el usurio completa el campo First Name con su nombre <nombre>
    And el usuario completa el campo Last Name con su apellido <apellido>
    And el usuario completa el campo Zip/Postal Code con su codigo postal <codPostal>
    And el usuario hace click en el boton continuar
    Then el usuario visualiza el mensaje <mensajeError>
    Examples:
      | nombre | apellido | codPostal | mensajeError                   |
      |        | Barrios  | 5000      | Error: First Name is required  |
      | Tobias |          | 5000      | Error: Last Name is required   |
      | Tobias | Barrios  |           | Error: Postal Code is required |
